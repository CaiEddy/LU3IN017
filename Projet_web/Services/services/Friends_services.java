...
public class Friends_services {
	...
	public static JSONObject addFriend(String id1, String id2) throws Exception {
		...
		// OUVRIR LA CONNECTION A LA BD avec CONNEXION_BD.JAVA
		...
		if(!Authentification_tools.test_token(id1)){
			...
			return ErrorJSON.serviceRefused("Le TOKEN de l'ID("+id1+") n'existe pas pour la methode getUser", -1);
		}
		...
		if(Friends_tools.exists_friend(id1, id2)){
			...
			return ErrorJSON.serviceRefused("id("+id1+") est deja l'ami de  id("+id2+") pour la methode addFriend", 1);
			}
		
		
		// Verifie si la creation de la relation de id1 vers id2 dans la base de donnee se fait correctement
		...
		if(!Friends_tools.mise_a_jour_nb_ami_commun(id1, id2)) {
			return ErrorJSON.serviceRefused("erreur lors de la mise a jour des nombres amis commun dans la table pour la methode addFriend", 1);
		}
		
		
		// Acces aux informations de la relation
		ArrayList<String> information = Mini_tools.requeteGET("Select * From "+Friends_tools.table+" Where id1='"+id1+"' AND id2='"+id2+"' ;");
		// Acces aux nom des attributs des informations
		ArrayList<String> nom_attributs = Mini_tools.acces_nom_attributs("Select * From "+Friends_tools.table+" Where id1='"+id1+"' AND id2='"+id2+"' ;");
		
		
		// FERMER LA CONNECTION A LA BD avec CONNEXION_BD.JAVA
	...
	public static JSONObject deleteFriend(String id1, String id2) throws JSONException, Exception {
		...
		if(!(Friends_tools.exists_friend(id1, id2))){
			...
			return ErrorJSON.serviceRefused("id("+id1+") n'est pas ami avec id("+id2+") pour la methode deleteFriend", 1);
		}
		...
		// Verifie si la mise a jours des nb ami commun se fait correct
		if(Friends_tools.mise_a_jour_nb_ami_commun(id1, id2)) {
			return ErrorJSON.serviceRefused("erreur lors de la mise a jour des nombres amis commun dans la table pour la methode deleteFriend", 1);
		}
	...
	public static JSONObject getFriendList(String id) throws Exception {
		...
		ArrayList<String> information = Mini_tools.requeteGET("Select id2,pseudo "
				+ "From "+User_tools.table+","+Friends_tools.table+" "
				+ "Where "+Friends_tools.table+".id2="+User_tools.table+".id "
				+ "AND "+Friends_tools.table+".id1='"+id+"' ;");
		// Acces aux nom des attributs des informations
		ArrayList<String> nom_attributs = Mini_tools.acces_nom_attributs("Select id2,pseudo "
				+ "From "+User_tools.table+","+Friends_tools.table+" "
				+ "Where "+Friends_tools.table+".id2="+User_tools.table+".id "
				+ "AND "+Friends_tools.table+".id1='"+id+"' ;");
